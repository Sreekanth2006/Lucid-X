...
async function loadEmotionModels() {
    try {
        ...
        const modelSources = [
            ...
            'https://cdnjs.cloudflare.com/ajax/libs/face-api.js/0.22.2/models'
        ];
        ...
        for (const modelUri of modelSources) {
            try {
                console.log(`🔄 Attempting to load models from: ${modelUri}`);
                ...
                console.log('✅ Emotion recognition models loaded successfully from:', modelUri);
                modelsLoaded = true;
                break;
            } catch (error) {
                console.warn(`⚠️ Failed to load from ${modelUri}:`, error.message);
                lastError = error;
            }
        }
        ...
        emotionStatus.textContent = 'Manual Mode (AI Unavailable)';
        emotionStatus.style.color = '#f59e0b';
        
        // Show manual emotion selector as fallback
